...
fn handle_client(mut stream: TcpStream, clients: Arc<Mutex<Vec<TcpStream>>>) {
    ...
    if let Ok(bytes_read) = stream.read(&mut buffer) {
        if bytes_read > 0 {
            client_name = String::from_utf8_lossy(&buffer[..bytes_read]).to_string();
    ...
    loop {
        match stream.read(&mut buffer) {
            Ok(bytes_read) => {
                ...
                broadcast_message(&clients, format!("{}: {}", client_name, message));
            }
            Err(_) => {
                break;
            }
...
fn broadcast_message(clients: &Arc<Mutex<Vec<TcpStream>>>, message: String) {
